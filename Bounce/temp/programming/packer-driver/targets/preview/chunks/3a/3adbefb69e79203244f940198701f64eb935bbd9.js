  ...
  return {
    ...
    execute: function () {
      ...
      _export("GameMgr", GameMgr = (_dec = ccclass('GameMgr'), _dec(_class = (_class2 = class GameMgr extends Component {
        ...
        generateBlocks() {
          var blockRankList = (_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
            error: Error()
          ...
          if (blockRankList.size > 0) {
            ...
            var maxRowIndex = Math.max(...blockRankList.keys());
          ...
          var newRowBlocks = this.createNewRow(0);
        ...
        moveBlocksUp() {
          ...
          var newRankList = new Map(); // 遍历所有排，将每排的块向上移动

          blockRankList.forEach((rankInfo, rowIndex) => {
            var newRowIndex = rowIndex + 1; // 移动该排所有块的位置

            rankInfo.blocks.forEach(blockNode => {
              var currentPos = blockNode.worldPosition.clone();
        ...
        createNewRow(rowIndex) {
          ...
          var blockCount = Math.floor(Math.random() * 6) + 1; // 块的宽度
