    ...
    execute: function () {
      ...
      _export("UIMgr", UIMgr = (_dec = ccclass('UIMgr'), _dec(_class2 = (_class3 = class UIMgr {
        ...
        async Open(uiOpt, data, afterClose, openCb) {
          ...
          try {
            if (!uiNode) {
              let uiPrefab = (_crd && GM === void 0 ? (_reportPossibleCrUseOfGM({
                error: Error()
              }), GM) : GM).ResMgr.Res.get(uiOpt.prefab, Prefab);

              if (!uiPrefab) {
                uiPrefab = await (_crd && GM === void 0 ? (_reportPossibleCrUseOfGM({
                  error: Error()
              ...
              if (this.layers[uiComp.uiType]) {
                ...
                if (this.uiInfos[uiNode.uuid]) {
                  ...
                  let ui = null; // if (uiOpt.script) {
                  //   ui = uiNode.addComponent(uiOpt.script);
                  // }

                  ui = uiNode.getComponent(_crd && UIComponent === void 0 ? (_reportPossibleCrUseOfUIComponent({
                    error: Error()
                  }), UIComponent) : UIComponent);
                  layer.addChild(uiNode);
                  ret.ui = ui;
                  ret.completed(ui); // GM.EventMgr.Emit(GM.EventMgr.EventList.UI_TYPE_UI_OPEN, uiNode);
        ...
        CheckUIPropertyOnUIOpen(uiNode) {
          ...
          const uiBase = uiNode.getComponent(_crd && UIComponent === void 0 ? (_reportPossibleCrUseOfUIComponent({
            error: Error()
          }), UIComponent) : UIComponent);
        ...
        */
        ...
        UpdateUIActiveByUIProperty(targetUiNode, active) {
          ...
          const uiBase = targetUiNode.getComponent(_crd && UIComponent === void 0 ? (_reportPossibleCrUseOfUIComponent({
            error: Error()
          }), UIComponent) : UIComponent);
